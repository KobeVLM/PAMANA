...
| Description      | A parent or learner creates a new Grade 2 learner account. Optionally joins a Klase via teacher-provided join code. Spring Security JWT Authentication creates the auth account, the users table is populated, and module_progress is initialized with Module 1 unlocked and Modules 2-4 locked.                                                                                                                                                                                                                      |
| Precondition     | No existing account for the provided email. Registration page accessible via browser.                                                                                                                                                                                                                                                                                                                                                                                                            |
| Normal Flow      | 1\. Navigate to registration page. <br>2\. Enter learner name, email, password. <br>3\. Optionally enter Klase join code. <br>4\. Submit form. <br>5\. System calls Spring Security JWT Authentication to create auth account. <br>6\. System inserts users record (role = 'learner'). <br>7\. System initializes 4 module_progress records (Module 1 is_unlocked = TRUE; Modules 2-4 FALSE). <br>8\. If valid Klase code provided, users.klase_id updated. <br>9\. Redirect to Pamana Trail with Lola welcome audio. |
...
| Description      | Registered user logs in via email and password. System authenticates via Spring Security JWT Authentication and redirects based on role: Learner → Pamana Trail; Parent → Dashboard; Teacher → Klase Dashboard.                                                                                                         |
| Precondition     | Existing registered account. Login page accessible.                                                                                                                                                                                                                                                |
| Normal Flow      | 1\. Navigate to login page. <br>2\. Enter email and password. <br>3\. Submit. <br>4\. React calls Spring Boot local auth API (/api/auth/login)(). <br>5\. local database returns JWT. <br>6\. System reads role from users table. <br>7\. Redirect by role: Learner → /trail, Parent → /dashboard, Teacher → /klase. |
